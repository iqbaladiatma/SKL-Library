<x-guest-layout>
    <div class="bg-gradient-to-br from-emerald-900 to-teal-900 min-h-screen flex items-center">
        <div class="max-w-md w-full mx-auto p-6 bg-white/5 backdrop-blur-lg rounded-2xl border border-white/10 shadow-xl">
            <!-- Session Status -->
            <x-auth-session-status class="mb-6 text-emerald-400" :status="session('status')" />
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <div class="text-center mb-8">
                    ...
                    <h2 class="mt-4 text-3xl font-bold text-white">Welcome Back</h2>
                </div>
                ...
                <div class="flex items-center justify-between">
                    <label for="remember_me" class="flex items-center">
                        <input id="remember_me" type="checkbox" class="rounded border-white/20 text-emerald-500 focus:ring-emerald-500" name="remember">
                        <span class="ml-2 text-sm text-emerald-200">{{ __('Remember me') }}</span>
                    </label>

                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-sm text-emerald-300 hover:text-emerald-400 underline">
                            {{ __('Forgot password?') }}
                        </a>
                    @endif
                </div>
                ...
                <p class="text-center text-emerald-300 text-sm mt-6">
                    ...
                    <a href="{{ route('register') }}" class="text-white hover:text-emerald-400 font-semibold underline">
                        {{ __('Register here') }}
                    </a>
                </p>
            </form>
